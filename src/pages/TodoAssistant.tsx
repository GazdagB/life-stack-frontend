...
import {
  Bot, CheckCircle2, ChevronDown, CircleDashed, ClipboardCheck, Info, ListChecks,
...
export default function TodoAssistant() {
  ...
  return (
    <>
      ...
      <Collapsible className="group/privacy">
        <Card className="border-violet-200 bg-violet-50/60 shadow-none">
          <CardContent className="py-3">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1"><p className="font-medium text-violet-950">{t("privacyTitle")}</p><p className="truncate text-sm text-violet-900/65">{t("privacySummary")}</p></div>
              <CollapsibleTrigger asChild><Button variant="ghost" size="sm" className="shrink-0 text-violet-900">{t("details")}<ChevronDown className="transition-transform group-data-[state=open]/privacy:rotate-180" /></Button></CollapsibleTrigger>
            </div>
            <CollapsibleContent><p className="mt-3 border-t border-violet-200 pt-3 text-sm leading-relaxed text-violet-900/70">{t("privacyDescription")}</p></CollapsibleContent>
          </CardContent>
        </Card>
      </Collapsible>
      ...
      <div className="flex flex-col gap-3 rounded-xl border bg-card p-3 shadow-xs lg:flex-row lg:items-end lg:justify-between">
        <div className="flex flex-wrap gap-2">
          {summary.map(({ label, value, icon: Icon, style }) => (
            <div key={label} className="flex items-center gap-2 rounded-lg border bg-background px-3 py-2">
              <span className={cn("flex size-7 items-center justify-center rounded-md", style)}><Icon className="size-3.5" /></span>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {filteredTodos.map((todo) => {
          ...
          if (!assessment) return (
            <Card key={todo.id} className="border-l-4 border-l-slate-300" size="sm">
              <CardHeader><div className="flex items-start justify-between gap-3"><div className="min-w-0"><CardTitle>{todo.title}</CardTitle><CardDescription className="mt-1 line-clamp-1">{todo.description || t("unassessedHelp")}</CardDescription></div><Badge variant="secondary" className="shrink-0"><CircleDashed className="mr-1 size-3" />{t("unassessed")}</Badge></div></CardHeader>
            </Card>
          )
          ...
          return (
            <Collapsible key={todo.id} className="group/assessment">
            <Card size="sm" className={cn("border-l-4", config.panel, assessment.is_stale && "opacity-75")}>
              <CardHeader>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0"><CardTitle>{todo.title}</CardTitle>{todo.description && <CardDescription className="mt-1 line-clamp-1">{todo.description}</CardDescription>}</div>
              ...
              <CardContent className="space-y-3">
                {assessment.is_stale && <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900"><RefreshCw className="size-4 shrink-0" /><span className="font-medium">{t("stale")}</span></div>}
                <p className="line-clamp-2 text-sm leading-relaxed text-muted-foreground">{assessment.reason}</p>
                <div className="flex flex-wrap items-center justify-between gap-2 border-t pt-3">
                  <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground"><span>{t("confidence", { count: assessment.confidence })}</span><span>·</span><span>{assessment.assessment_source === "AI" ? t("sourceAI") : t("sourceRules")}</span></div>
                  <div className="flex items-center gap-2">
                    <CollapsibleTrigger asChild><Button variant="ghost" size="sm">{t("details")}<ChevronDown className="transition-transform group-data-[state=open]/assessment:rotate-180" /></Button></CollapsibleTrigger>
                    {!!assessment.ai_steps.length && !assessment.is_stale && <Button size="sm" asChild><Link to={`/todos/assistant/${todo.id}`}><Bot />{t("doYourPart")}</Link></Button>}
                  </div>
                </div>
                <CollapsibleContent>
                  <div className="space-y-4 border-t pt-4">
                    {assessment.is_stale && <p className="text-sm text-amber-800">{t("staleHelp")}</p>}
