...
function Poster({ src, title, className }: { src: string | null; title: string; className?: string }) {
  ...
  if (src) {
    return <img src={src} alt={t("posterAlt", { title })} loading="lazy" className={cn("bg-muted object-cover", className)} />
  }
  return (
    <div className={cn("flex items-center justify-center bg-muted text-muted-foreground", className)} aria-label={t("noPoster", { title })}>
...
function StatusBadge({ status }: { status: MovieListStatus }) {
  ...
  return status === "WATCHED"
    ? <Badge className="gap-1 bg-emerald-100 text-emerald-800"><Check className="size-3" />{t("watched")}</Badge>
    : <Badge variant="secondary" className="gap-1"><Bookmark className="size-3" />{t("want")}</Badge>
}
      ...
      {movies.map((movie) => (
        <button key={movie.id} type="button" onClick={() => onSelect(movie)} className="group overflow-hidden rounded-xl border bg-card text-left shadow-xs transition hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50">
          ...
          <div className="p-3">
            ...
            <div className="mt-1 flex items-center justify-between gap-2 text-xs text-muted-foreground"><span>{movie.year ?? t("unknownYear")}</span><span>{movie.runtime ?? ""}</span></div>
          </div>
        </button>
      ))}
    </div>
...
function MovieSheet({ details, movie, isLoading, onClose, onAdd, onUpdate, onDelete }: MovieSheetProps) {
  ...
  async function run(action: () => Promise<void>) {
    ...
    setIsSaving(true)
    try { await action() }
    catch (reason) { setLocalError(reason instanceof Error ? reason.message : t("errors.save")) }
          ...
          <>
            ...
            <div className="space-y-6 px-4 pb-4">
              ...
              <section className="space-y-2">
                <h3 className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">{t("critics")}</h3>
                <RatingTiles ratings={item.external_ratings} />
                <p className="text-[11px] text-muted-foreground">{t("ratingsNote")}</p>
              </section>
              {item.plot && <section><h3 className="mb-2 text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">{t("story")}</h3><p className="leading-6 text-foreground/85">{item.plot}</p></section>}
              <dl className="grid gap-3 text-sm sm:grid-cols-2">
                {item.director && <div><dt className="text-xs text-muted-foreground">{t("director")}</dt><dd>{item.director}</dd></div>}
                {item.genre && <div><dt className="text-xs text-muted-foreground">{t("genre")}</dt><dd>{item.genre}</dd></div>}
                {item.released && <div><dt className="text-xs text-muted-foreground">{t("released")}</dt><dd>{item.released}</dd></div>}
                {item.country && <div><dt className="text-xs text-muted-foreground">{t("country")}</dt><dd>{item.country}</dd></div>}
                {item.language && <div><dt className="text-xs text-muted-foreground">{t("language")}</dt><dd>{item.language}</dd></div>}
                {item.box_office && <div><dt className="text-xs text-muted-foreground">{t("boxOffice")}</dt><dd>{item.box_office}</dd></div>}
                {item.actors && <div className="sm:col-span-2"><dt className="text-xs text-muted-foreground">{t("cast")}</dt><dd>{item.actors}</dd></div>}
                {item.awards && <div className="sm:col-span-2"><dt className="text-xs text-muted-foreground">{t("awards")}</dt><dd>{item.awards}</dd></div>}
              </dl>

              {movie && (
                <form onSubmit={(event) => { event.preventDefault(); void run(() => onUpdate(draft)) }} className="space-y-4 rounded-xl border bg-muted/20 p-4">
                  <div>
                    <h3 className="font-medium">{t("noteTitle")}</h3>
                    <p className="text-xs text-muted-foreground">{t("noteHelp")}</p>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <Button type="button" variant={draft.list_status === "WANT_TO_WATCH" ? "default" : "outline"} onClick={() => changeStatus("WANT_TO_WATCH")}><Bookmark />{t("want")}</Button>
                    <Button type="button" variant={draft.list_status === "WATCHED" ? "default" : "outline"} onClick={() => changeStatus("WATCHED")}><Check />{t("watched")}</Button>
                  </div>
                  {draft.list_status === "WATCHED" && (
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div className="space-y-2"><label className="text-sm font-medium" htmlFor="movie-rating">{t("yourScore")}</label><div className="relative"><Star className="absolute left-2.5 top-2 size-4 fill-amber-400 text-amber-400" /><Input id="movie-rating" className="pl-8" type="number" min="1" max="10" step="0.5" value={draft.personal_rating ?? ""} onChange={(event) => setDraft({ ...draft, personal_rating: event.target.value || null })} placeholder="1–10" /></div></div>
            ...
            {!movie && (
              <SheetFooter className="sticky bottom-0 border-t bg-popover/95 backdrop-blur">
                {localError && <p className="text-sm text-destructive">{localError}</p>}
                <div className="grid grid-cols-2 gap-2">
                  <Button variant="outline" onClick={() => void run(() => onAdd("WANT_TO_WATCH"))} disabled={isSaving}><Bookmark />{t("want")}</Button>
                  <Button onClick={() => void run(() => onAdd("WATCHED"))} disabled={isSaving}>{isSaving ? <LoaderCircle className="animate-spin" /> : <Check />}{t("markWatched")}</Button>
                </div>
              </SheetFooter>
            )}
          </>
...
export default function Movies() {
  ...
  async function searchMovies(event: React.FormEvent) {
    ...
    catch (reason) { setError(reason instanceof Error ? reason.message : t("errors.search")); setResults([]) }
  ...
  async function openResult(result: MovieSearchResult) {
    if (result.library_id) {
      ...
      catch (reason) { setError(reason instanceof Error ? reason.message : t("errors.saved")) }
    ...
    catch (reason) { setError(reason instanceof Error ? reason.message : t("errors.details")) }
    finally { setIsLoadingDetails(false) }
  }
  ...
  async function generateRecommendation() {
    ...
    catch (reason) { setError(reason instanceof Error ? reason.message : t("errors.recommendation")) }
      ...
      {mode === "suggestions" ? (
        <>
          <Card className="relative overflow-hidden bg-gradient-to-br from-violet-50 via-card to-indigo-50 dark:from-violet-950/25 dark:via-card dark:to-indigo-950/25">
            ...
            <CardContent className="relative grid items-center gap-6 py-4 md:grid-cols-[1fr_auto]">
              <div className="flex items-start gap-4">
                ...
                <div><p className="font-semibold">{t("tasteTitle")}</p><p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">{t("tasteDescription")}</p></div>
              </div>
              <div className="flex flex-col items-stretch gap-2 md:items-end">
                <Button size="lg" onClick={() => void generateRecommendation()} disabled={isLoading || isRecommending || ratedMovieCount < 3}>
                  ...
                  {isRecommending ? t("finding") : recommendations.length > 0 ? t("anotherSet") : t("suggest")}
                </Button>
                <p className="text-center text-xs text-muted-foreground md:text-right">{t("ratedAvailable", { count: Math.min(ratedMovieCount, 10) })}</p>
              </div>
            </CardContent>
          </Card>
            ...
            return (
              <div className="space-y-4">
                <Card className="overflow-hidden py-0">
                  <div className="grid md:grid-cols-[280px_1fr]">
                    ...
                    <div className="flex flex-col p-6 sm:p-8">
                      ...
                      <div className="mt-6"><p className="mb-2 text-xs font-medium text-muted-foreground">{t("basedOn")}</p><div className="flex flex-wrap gap-2">{recommendation.based_on.slice(0, 5).map((movie) => <span key={movie.title} className="inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs"><Star className="size-3 fill-amber-400 text-amber-400" />{movie.title} · {Number(movie.personal_rating).toFixed(1)}</span>)}</div></div>
                      <div className="mt-auto flex justify-end pt-8"><Button onClick={() => setSelectedDetails(recommendation)}>{t("viewAdd")}</Button></div>
                    </div>
                  </div>
                </Card>
                ...
                {alternatives.length > 0 && (
                  <div>
                    <div className="mb-3 flex items-end justify-between gap-4"><div><h3 className="font-semibold">{t("moreMatches")}</h3><p className="text-sm text-muted-foreground">{t("alternatives")}</p></div><p className="hidden text-xs text-muted-foreground sm:block">{t("inspect")}</p></div>
                      ...
                      {alternatives.map((movie, index) => (
                        <Card key={movie.imdb_id} className="group cursor-pointer overflow-hidden py-0 transition-all hover:-translate-y-0.5 hover:border-violet-300 hover:shadow-md" onClick={() => setSelectedDetails(movie)}>
                          <div className="grid min-h-48 grid-cols-[112px_1fr] md:min-h-64 md:grid-cols-1 md:grid-rows-[180px_1fr]">
                            <div className="relative bg-muted"><Poster src={movie.poster_url} title={movie.title} className="absolute inset-0 size-full" /><Badge className="absolute left-2 top-2 bg-black/60 text-white backdrop-blur">{t("match", { number: index + 2 })}</Badge></div>
          ...
          {!isRecommending && recommendations.length === 0 && !error && (
            <Card className="border-dashed py-16"><CardContent className="flex flex-col items-center text-center"><div className="mb-4 rounded-2xl bg-muted p-4"><Sparkles className="size-7 text-muted-foreground" /></div><p className="font-medium">{t("suggestionEmptyTitle")}</p><p className="mt-1 max-w-md text-sm text-muted-foreground">{t("suggestionEmptyDescription")}</p></CardContent></Card>
          )}
        </>
      ) : mode === "discover" ? (
        <>
          <Card className="bg-gradient-to-br from-card to-muted/40">
            <CardHeader><CardTitle className="flex items-center gap-2"><Sparkles className="size-4" />{t("findTitle")}</CardTitle><CardDescription>{t("findDescription")}</CardDescription></CardHeader>
            <CardContent>
              <form onSubmit={searchMovies} className="flex gap-2">
                <div className="relative flex-1"><Search className="absolute left-3 top-2.5 size-4 text-muted-foreground" /><Input className="h-10 pl-9 text-base" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("queryPlaceholder")} aria-label={t("movieTitle")} autoFocus /></div>
                <Button size="lg" className="h-10 px-4" disabled={isSearching || query.trim().length < 2}>{isSearching ? <LoaderCircle className="animate-spin" /> : <Search />}{t("search")}</Button>
              </form>
            </CardContent>
          </Card>

          {hasSearched && <div className="flex items-end justify-between"><div><h2 className="text-lg font-semibold">{t("results")}</h2><p className="text-sm text-muted-foreground">{isSearching ? t("searching") : t("matching", { count: resultCount.toLocaleString(i18n.resolvedLanguage) })}</p></div></div>}
          {!hasSearched && (
            <div className="grid gap-4 md:grid-cols-3">
              {[
                [Search, t("featureSearch"), t("featureSearchHelp")],
                [Bookmark, t("featureQueue"), t("featureQueueHelp")],
                [Star, t("featureReview"), t("featureReviewHelp")],
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {results.map((result) => (
                <button key={result.imdb_id} type="button" onClick={() => void openResult(result)} className="group overflow-hidden rounded-xl border bg-card text-left shadow-xs transition hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50">
                  ...
                  <div className="p-3"><p className="truncate font-medium">{result.title}</p><p className="mt-1 text-xs text-muted-foreground">{result.year ?? t("unknownYear")}</p></div>
                </button>
              ))}
            </div>
          )}
          {hasSearched && !isSearching && results.length === 0 && <Card className="border-dashed"><CardContent className="py-12 text-center text-sm text-muted-foreground">{t("noMatches")}</CardContent></Card>}
        </>
      ) : isLoading ? (
